...
class XiaoyunDetail:
    ...
    def printData(self):
        print(f"printData:小运[{self.start_year}年---{self.ganzhi}---{self.ganzhi_shishen}"
              f"---{self.rizhi_shiergong}---{self.nayin}---{self.kongwang}")


# 按照 问真八字 页面进行设计排版
class XiaoyunWidget(QWidget):
    """自定义多文本显示部件"""

    def __init__(self, xiaoyunDetail: XiaoyunDetail, parent=None):
        super().__init__(parent)

        self.xiaoyunDetail = xiaoyunDetail
        ...
        if xiaoyunDetail.ganzhi:  # 不为空
            # for item_data in text_items:
            #     self.add_text_item(item_data)
            self.label_start_year = QLabel(xiaoyunDetail.start_year)
            ...
            hLayout = QHBoxLayout()
            self.label_xiaoyun_ganzhi = VerticalTextLabel(xiaoyunDetail.ganzhi)
            self.label_shishen_ganzhi = VerticalTextLabel(xiaoyunDetail.ganzhi_shishen)
    ...
    def get_xiaoyun_data(self):
        return self.xiaoyunDetail


class XiaoYunTableHandler:
    """表格处理工具类，用于操作从UI加载的表格"""

    @staticmethod
    def set_text_cell(table_widget, row, col, xiaoyunDetail: XiaoyunDetail):
        """给表格单元格设置多文本内容"""
        if not isinstance(table_widget, QTableWidget):
            raise ValueError("传入的不是QTableWidget对象")

        widget = XiaoyunWidget(xiaoyunDetail)
